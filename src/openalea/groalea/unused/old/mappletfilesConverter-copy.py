...
def uppermetamerLevelConvert(mtg, rootedgraph):
    ...
    for scale in range(max_scale):
        for vid in mtg.vertices(scale + 1):
            ...
            rootedgraph = setVetexProperties(vid, sid, mtg, rootedgraph)
...
def convert(mtgfile, bgeomfile):
    ...
    #rootedgraph = uppermetamerLevelConvert(mtg, rootedgraph)

    for vid in vlist:
        # for test
        if vlist.index(vid) == 2:
            vtest = vlist[vlist.index(vid) - 1]
            break
...
def setVetexProperties(vid, sid, mtg, rootedgraph):
...
def metamerLevelConvert(vid, mtg, rootedgraph):
    ...
    vcomplex = mtg.complex(vid)
    scomplex = vcomplex * 10**offset
    if scomplex not in rootedgraph:
    ...
    while vcomplex != 0:
        tmpvid = vcomplex
        tmpsid = tmpvid * 10**offset

        if tmpsid not in rootedgraph:
            rootedgraph.add_vertex(tmpsid)
            rootedgraph = setVetexProperties(tmpvid, tmpsid, mtg, rootedgraph)

        vcomplex = mtg.complex(tmpvid)
        scomplex = vcomplex * 10**offset
        ...
        if vparent != None:
            sparent = vparent * 10**offset
            etype = mtg.edge_type(tmpvid)
            addEdge2RootedGraph(rootedgraph, sparent, tmpsid, etype)
